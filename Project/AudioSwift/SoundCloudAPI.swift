...
class SoundCloudAPI {
    ...
    func searchSoundCloudSongs(_ searchTerm: String?, completionHandler: @escaping (_ resultArray: [AnyHashable]?, _ error: String?) -> Void) {
        ...
        let apiURL = "http://api.soundcloud.com/tracks?client_id=\(AppDelegate.soundCloudAPI() ?? "")&q=\(searchTerm ?? "")&format=json"
    ...
    func soundCloudSongs(_ param: String?, type: String?, offset: Int, limit: Int, completionHandler: @escaping (_ resultArray: [AnyHashable]?, _ error: String?) -> Void) {
        ...
        if type?.isEqual("user") ?? false {
            apiURL = String(format: "http://api.soundcloud.com/users/%@/tracks?client_id=%@&offset=%i&limit=%i&format=json", param ?? "", SOUNDCLOUD_CLIENT, offset, limit)
        } else {
            apiURL = String(format: "http://api.soundcloud.com/playlists/%@/tracks?client_id=%@&offset=%i&limit=%i&format=json", param ?? "", SOUNDCLOUD_CLIENT, offset, limit)
        }
