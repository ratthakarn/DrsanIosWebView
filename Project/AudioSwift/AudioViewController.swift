...
@objc final class AudioViewController: UICollectionViewController, UISearchBarDelegate{
    
    @objc var params: NSArray!
    @objc var isWordpress: Bool = false
        ...
        if (isWordpress) {
                ...
                if let results = posts {
                    ...
                    for result in results {
                        ...
                        if (!post.attachmentsIncomplete) {
                            ...
                            //TODO posts from Rest API that may contain audio but that are not yet completed!!
                        }
                    }
                    
                    self.handleResults(resultContainsEntries: (results.count == 0))
                }
            ...
            SoundCloudAPI.sharedInstance()!.soundCloudSongs(params![0] as? String, type: params![1] as? String, offset: (page - 1) * perPage, limit: perPage, completionHandler: { resultArray, error in
                ...
                if (resultArray == nil) {
                    ...
                    self.handleResults(resultContainsEntries: (resultArray?.count ?? 0) < 1)
    ...
    func parseSongFromPost(post: WPPost, audioAtt: WPAttachment?) -> SoundCloudSong {
        ...
        if let medium = post.featured_media.sizes.medium {
            song.artWorkURL = medium
        }
        if let high = post.featured_media.sizes.large {
    ...
    func handleNoResults(){
        let alertController = UIAlertController.init(title: NSLocalizedString("error", comment: ""), message: NO_CONNECTION_TEXT, preferredStyle: UIAlertController.Style.alert)
    ...
    func handleResults(resultContainsEntries: Bool){
        if (!resultContainsEntries) {
