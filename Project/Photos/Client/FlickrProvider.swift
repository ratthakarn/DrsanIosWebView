...
class FlickrProvider: PhotosProvider {
    func getRequestUrl(params: [String], page: Int) -> String? {
        ...
        return String(format:"https://api.flickr.com/services/rest/?method=flickr.%@.getPhotos&api_key=%@&%@=%@&format=json&extras=path_alias,url_o,url_c,url_b,url_z&per_page=20&page=%d", pathMethod, AppDelegate.flickrAPI(), idMethod, galleryId, page)
    }
