        ...
        if (forType is WPPost.Type) { 
            ...
            if let posts = parseable.array {
                for post in posts {
                    ...
                    result.featured_media.url = post["_embedded"]["wp:featuredmedia"].array?[0]["source_url"].string
                    result.thumbnail.url = post["_embedded"]["wp:featuredmedia"].array?[0]["media_details"]["sizes"]["medium"]["source_url"].string
                        ...
                        Alamofire.request(attachmentsUrl).responseJSON { (responseData) -> Void in
                            if((responseData.result.value) != nil) {
                                ...
                                for attachmentJson in attachments.arrayValue {
                                    ...
                                    if let attachmentUrl = attachmentJson["media_details"]["sizes"]["full"]["source_url"].string {
                                    ...
                                    if let attachmentLarge = attachmentJson["thumbnails"]["large"].string {
                                        attachment.sizes.large = attachmentLarge
                                    }
                                    if let attachmentMedium = attachmentJson["media_details"]["sizes"]["medium"]["source_url"].string {
                                        attachment.sizes.medium = attachmentMedium
                                    }
                                    if let attachmentThumbnail = attachmentJson["media_details"]["sizes"]["thumbnail"]["source_url"].string {
