...
@objc final class WordpressSwiftViewController: UICollectionViewController, UISearchBarDelegate{
    ...
    func shouldShowHeader()-> Bool {
        return (self.items.count > 0 && self.items[0].featured_media.url != nil)
    }
    
    func shouldShowCategories() -> Bool {
        //If the list type is immersive, or if the first post has no image, no category slider
        if (postType == PostCellImmersive.identifier) { return false }
        if (self.items.count > 0 && self.items[0].featured_media.url == nil) { return false }
