...
@property (nonatomic) CGFloat topViewBackgroundDefaultY;
...
- (CGFloat)topViewBackgroundDefaultY
{
    if ([[UIApplication sharedApplication] statusBarFrame].size.height == 0.0f) {
        if ([self.topViewBackground isKindOfClass:[UINavigationBar class]]) {
            return _topViewBackgroundDefaultY - [[UIApplication sharedApplication] statusBarFrame].size.height - JDFPeekabooCoordinatorNavigationBarHorizontalHeightDifferential;
        } else {
            return _topViewBackgroundDefaultY - [[UIApplication sharedApplication] statusBarFrame].size.height;
        }
    } else {
        return _topViewBackgroundDefaultY;
    }
}
...
- (CGFloat)topViewDefaultY
{
    if ([[UIApplication sharedApplication] statusBarFrame].size.height == 0.0f) {
        if ([self.topView isKindOfClass:[UINavigationBar class]]) {
            return _topViewDefaultY - [self defaultStatusBarHeight];
        } else {
            return _topViewDefaultY - [self defaultStatusBarHeight];
        }
    } else {
...
{
    _topView = topBar;
    self.topViewDefaultY = [self defaultStatusBarHeight];
}

- (void)setTopViewBackground:(UIView *)topBarBackground
{
    _topViewBackground = topBarBackground;
    self.topViewBackgroundDefaultY = topBarBackground.frame.origin.y;
}
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.topViewMinimisedHeight = [self defaultStatusBarHeight];
        self.topViewBackgroundMinimisedHeight = [self defaultStatusBarHeight];
...
- (void)fullyExpandViews
{
    ...
    [self animateTopViewBackgroundToYPosition:self.topViewBackgroundDefaultY];
...
- (void)fullyHideViews
{
    ...
    [self animateTopViewBackgroundToYPosition:[self topViewBackgroundMinimisedY]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    CGRect topBarBackgroundFrame = self.topViewBackground.frame;
    ...
    CGFloat topBarBackgroundHeight = topBarBackgroundFrame.size.height;
    if ([[UIApplication sharedApplication] statusBarFrame].size.height == 0.0f) {
        topBarBackgroundHeight += self.topViewMinimisedHeight;
    }
    ...
    if (scrollOffset <= -scrollView.contentInset.top) {
        ...
        topBarBackgroundFrame.origin.y = self.topViewBackgroundDefaultY;
        bottomBarFrame.origin.y = defaultBottomViewY;
    } else if ((scrollOffset + scrollHeight) >= scrollContentSizeHeight) {
        topBarFrame.origin.y = -topBarHeight + self.topViewMinimisedHeight;
        topBarBackgroundFrame.origin.y = -topBarBackgroundHeight + self.topViewBackgroundMinimisedHeight;
       ...
       topBarBackgroundFrame.origin.y = MIN(self.topViewBackgroundDefaultY, MAX(-topBarBackgroundHeight + self.topViewBackgroundMinimisedHeight, topBarBackgroundFrame.origin.y - scrollDiff));
    ...
    [self.topViewBackground setFrame:topBarBackgroundFrame];
    ...
    self.scrollView.scrollIndicatorInsets = UIEdgeInsetsMake(0, 0, bottom, 0);
...
- (void)animateTopViewBackgroundToYPosition:(CGFloat)y
{
    [UIView animateWithDuration:0.2 animations:^{
        CGRect backgroundFrame = self.topViewBackground.frame;
        backgroundFrame.origin.y = y;
        
        [self.topViewBackground setFrame:backgroundFrame];
    } completion:^(BOOL finished) {
        [self setBarsNeedDisplay];
    }];
}
...
- (CGFloat)topViewPercentageHidden
{
    ...
    if ([self isIphoneX])  {
        percentage = fabs(1 - fabs(frame.size.height - (frame.origin.y + self.topViewDefaultY)) / frame.size.height);
    } else {
        percentage = (self.topViewDefaultY - frame.origin.y - self.topViewDefaultY) / (frame.size.height - self.topViewMinimisedHeight);
    }
...
- (CGFloat)topViewBackgroundMinimisedY
{
    CGRect topViewBackgroundFrame = self.topViewBackground.frame;
    if ([[UIApplication sharedApplication] statusBarFrame].size.height == 0.0f) {
        return -topViewBackgroundFrame.size.height - [[UIApplication sharedApplication] statusBarFrame].size.height;
    } else {
        return -topViewBackgroundFrame.size.height + self.topViewBackgroundMinimisedHeight;
    }
}
...
//Return the height that the statusbar has (when not hidden)
- (CGFloat)defaultStatusBarHeight {
    if ([[UIApplication sharedApplication] statusBarFrame].size.height > 0) return [[UIApplication sharedApplication] statusBarFrame].size.height;
