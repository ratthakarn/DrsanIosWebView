...
class PinterestProvider: SocialProvider {
    func getRequestUrl(identifier: String, params: SocialRequestParams) -> String? {
        ...
        if let nextPageUrl = params.nextPageToken {
            ...
            query = String(format: "https://api.pinterest.com/v1/boards/%@/pins/?fields=id,original_link,note,image,media,attribution,created_at,creator(image,first_name),counts&limit=100&access_token=%@", identifier, AppDelegate.pinterestToken())
        }
