...
class InstagramProvider: SocialProvider {
    func getRequestUrl(identifier: String, params: SocialRequestParams) -> String? {
        ...
        let query = String(format: "https://graph.facebook.com/v3.1/%@/media?access_token=%@&date_format=U&fields=caption,id,ig_id,comments_count,timestamp,permalink,owner{profile_picture_url,name},media_url,media_type,like_count,comments,username,children{media_url,media_type},thumbnail_url%@", identifier, AppDelegate.instagramToken(), param).addingPercentEncoding( withAllowedCharacters: .urlQueryAllowed)!
