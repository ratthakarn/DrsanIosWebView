...
public class SocialItem: NSObject {
    ...
    convenience init(facebookPost: JSON) {
        ...
        if let link = facebookPost["link"].string {
            url = link
        } else {
        ...
        if facebookPost["type"].stringValue == "video", let video = facebookPost["source"].string {
            videoUrl = video
        }
        authorName = facebookPost["from"]["name"].string
        authorImageUrl = String(format:"https://graph.facebook.com/v3.1/%@/picture", facebookPost["from"]["id"].stringValue)
