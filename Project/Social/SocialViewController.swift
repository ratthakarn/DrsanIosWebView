...
@objc final class SocialViewController: UICollectionViewController {
    
    @objc var params: NSArray!
    @objc var provider: NSNumber!
    ...
    func loadPosts() {
        ...
        client?.get(identifier: params[0] as! String, params: requestParams, provider: provider!, completionHandler: { (success, posts, pageToken) in
            ...
            if let results = posts {
                ...
                var canLoadMore = true
                if (provider is FacebookProvider || provider is InstagramProvider) && pageToken == nil {
                    canLoadMore = false
