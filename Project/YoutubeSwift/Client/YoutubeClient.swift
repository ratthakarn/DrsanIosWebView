...
class YoutubeClient: NSObject {
    ...
    static func getResults(parameter : String, type : RequestType, search: String?, pageToken : String?, completion:@escaping (_ success: Bool, _ nextPageToken : String?, _ items : [Video]) -> Void){
        ...
        if (type == .query) {
            ...
            url = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=\(parameter)&q=\(queryReady!)&maxResults=20&key=\(AppDelegate.googleAPI()!)&pageToken=\(pageToken ?? "")"
        } else if (type == .playlist) {
            url = "https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=\(parameter)&maxResults=20&key=\(AppDelegate.googleAPI()!)&pageToken=\(pageToken ?? "")"
        } else if (type == .live) {
            url = "https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=\(parameter)&type=video&eventType=live&maxResults=20&key=\(AppDelegate.googleAPI()!)&pageToken=\(pageToken ?? "")"
        } else if (type == .channel) {
            url = "https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=\(parameter)&type=video&order=date&maxResults=20&key=\(AppDelegate.googleAPI()!)&pageToken=\(pageToken ?? "")"
        } else if (type == .related) {
            url = "https://www.googleapis.com/youtube/v3/search?part=snippet&relatedToVideoId=\(parameter)&type=video&key=\(AppDelegate.googleAPI()!)"
        }
        
        
        let safeString = url!.addingPercentEncoding( withAllowedCharacters: .urlQueryAllowed)
        print("Query: \(String(describing: safeString))")
        
        Alamofire.request(safeString!).validate().responseJSON { response in
